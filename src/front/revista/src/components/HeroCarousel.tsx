...
export default function HeroCarousel({ slides = [] as Slide[], fullScreen = false }: { slides?: Slide[]; fullScreen?: boolean }) {
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      setComputedHeight(Math.max(0, Math.floor(h)));
    };
  ...
  return (
    <section
      ...
      className={`relative w-full ${fullScreen ? 'h-screen' : ''}`}
    >
          ...
          {slides.map((s, i) => (
            <article key={i} className="w-full flex-shrink-0 relative">
              {s.image ? (
                  ...
                  {}
                  <Image src={s.image} alt={s.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" unoptimized />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent" />
                </div>
              ) : (
                <div className="h-72 sm:h-96 md:h-[520px] w-full bg-gradient-to-r from-[#F69A73] to-[#189F66]" />
              )}
              ...
              <div className={`absolute inset-0 ${fullScreen ? 'flex items-center' : 'flex items-end'}`}>
                <div className={`max-w-4xl w-full mx-auto p-6 md:p-12 text-white ${fullScreen ? 'text-center' : ''}`}>
                  {s.kicker && (
                    <div className={`text-sm uppercase tracking-wider text-[#F69A73] ${fullScreen ? 'mb-4' : 'mb-2'}`}>
                      {s.kicker}
                    </div>
                  )}
                  ...
                  {s.excerpt && (
                    <p className={`${fullScreen ? 'mt-4' : 'mt-2'} ${fullScreen ? 'text-lg md:text-xl' : 'text-sm md:text-base'} text-white/90 max-w-2xl ${fullScreen ? 'mx-auto' : ''}`}>
                      {s.excerpt}
                    </p>
                  )}
                  {s.href && (
                    <div className={`mt-4 ${fullScreen ? 'justify-center flex' : ''}`}>
                      <Link href={s.href} className="inline-block bg-white text-black px-4 py-2 rounded-md font-medium hover:opacity-90">
                        Leia a matéria
                      </Link>
                    </div>
                  )}
      ...
      {}
      <button onClick={prev} aria-label="Anterior" className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 backdrop-blur-sm p-2 rounded-full shadow hover:bg-white cursor-pointer">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M15 18L9 12L15 6" stroke="#111827" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>

      <button onClick={next} aria-label="Próximo" className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 backdrop-blur-sm p-2 rounded-full shadow hover:bg-white cursor-pointer">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
