...
export interface PendingItem { id: string; targetEntityId: string; targetType: string; status: 'AguardandoRevisao' | 'Aprovado' | 'Rejeitado' | 'Arquivado'; dateRequested: string; requesterUsuarioId: string; commentary: string; commandType: string; commandParametersJson: string; idAprovador?: string | null; dataAprovacao?: string | null; }
interface PendingCardProps { pending: PendingItem; staffList: StaffMember[]; onUpdate: () => void; isAdmin: boolean; }

const statusDisplay = { AguardandoRevisao: { text: 'Aguardando revisão', color: 'text-yellow-600' }, Aprovado: { text: 'Aprovado', color: 'text-green-600' }, Rejeitado: { text: 'Rejeitado', color: 'text-red-600' }, Arquivado: { text: 'Arquivado', color: 'text-gray-500' } };
const jobDisplay: Record<string, string> = { Administrador: 'Administrador', EditorChefe: 'Editor Chefe', EditorBolsista: 'Editor Bolsista', Aposentado: 'Aposentado(a)' };
...
export default function PendingCard({ pending, staffList, onUpdate, isAdmin }: PendingCardProps) {
    // ... (Logic same as previous, just fixing the image in Popover above) ...
    ...
    const [resolverPendencia, { loading }] = useMutation(RESOLVER_REQUISICAO_PENDENTE, {
        refetchQueries: [{ query: OBTER_PENDENTES, variables: { pagina: 0, tamanho: 20, status: 'AguardandoRevisao' } }, { query: OBTER_PENDENTES, variables: { pagina: 0, tamanho: 20, status: 'Aprovado' } }, { query: OBTER_PENDENTES, variables: { pagina: 0, tamanho: 20, status: 'Rejeitado' } }],
        onCompleted: (data) => { if (data.resolverRequisicaoPendente) toast.success('Pendência resolvida!'); else toast.error('Falha.'); onUpdate(); },
        onError: (err) => toast.error(`Erro: ${err.message}`)
    });

    const handleActionSubmit = () => {
        toast.loading('Processando...', { id: 'pending-action' });
        resolverPendencia({ variables: { pendingId: pending.id, isApproved: selectedAction === 'Aprovar' } }).finally(() => toast.dismiss('pending-action'));
    };
    ...
    return (
        <li className="card-std p-4" style={{ width: '98%', margin: '10px 1%' }}>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
                ...
                <span className="font-semibold">Condição:</span><span className={`font-medium ${statusDisplay[pending.status].color}`}>{statusDisplay[pending.status].text}</span>
                <span className="text-gray-300">|</span>
