...
export interface VolumeCardData { id: string; volumeTitulo: string; volumeResumo: string; imagemCapa?: { url: string; textoAlternativo: string; } | null; }
interface VolumeFormData { edicao: number; volumeTitulo: string; volumeResumo: string; m: MesVolume; n: number; year: number; status: StatusVolume; imagemCapa?: { url: string; alt: string; midiaID: string; } | null; artigoIds: string[]; }
...
export default function VolumeEditorialCard({ mode, initialData, onUpdate }: VolumeEditorialCardProps) {
    ...
    const [formData, setFormData] = useState<Partial<VolumeFormData>>({ volumeTitulo: initialData?.volumeTitulo || '', volumeResumo: initialData?.volumeResumo || '', status: StatusVolume.EmRevisao, m: MesVolume.Janeiro, n: 1, year: new Date().getFullYear(), artigoIds: [] });
    ...
    const [loadVolumeData, { loading: loadingData }] = useLazyQuery(OBTER_VOLUME_POR_ID, {
        ...
        onCompleted: (data) => {
            if (data.obterVolumePorId) {
                ...
                if (data.obterVolumePorId.artigoIds.length > 0) loadArtigoTitles({ variables: { ids: data.obterVolumePorId.artigoIds } });
            }
    ...
    const [criarVolume, { loading: loadingCreate }] = useMutation(CRIAR_VOLUME, { onCompleted: () => { toast.success("Volume criado!"); setIsModalOpen(false); onUpdate(); }, onError: (err) => { toast.error(err.message); setIsModalOpen(false); } });
    const [atualizarVolume, { loading: loadingUpdate }] = useMutation(ATUALIZAR_METADADOS_VOLUME, { onCompleted: () => { toast.success("Volume atualizado!"); setIsEditing(false); setIsModalOpen(false); onUpdate(); }, onError: (err) => { toast.error(err.message); setIsModalOpen(false); } });
    const loading = loadingData || loadingArtigos || loadingCreate || loadingUpdate;

    const handleEditClick = () => { if (initialData?.id) { loadVolumeData({ variables: { idVolume: initialData.id } }); setIsEditing(true); } };
    const handleCancel = () => { if (mode === 'create') onUpdate(); else setIsEditing(false); };
    const handleFormChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => { const { name, value } = e.target; if (name === 'n' || name === 'year' || name === 'edicao') setFormData(prev => ({ ...prev, [name]: parseInt(value) || 0 })); else setFormData(prev => ({ ...prev, [name]: value })); };
    const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onloadend = () => setNewCoverImage(reader.result as string); reader.readAsDataURL(file); } };
    ...
    const handleConfirmSubmit = (commentary: string) => {
        const input = { edicao: formData.edicao, volumeTitulo: formData.volumeTitulo, volumeResumo: formData.volumeResumo, m: formData.m, n: formData.n, year: formData.year, status: formData.status, artigoIds: formData.artigoIds, imagemCapa: newCoverImage && newCoverImage.startsWith('data:image') ? { midiaID: `capa-${Date.now()}`, url: newCoverImage, alt: `Capa` } : null };
        if (mode === 'create') { toast.loading('Criando...', { id: 'vol' }); criarVolume({ variables: { input, commentary } }).finally(() => toast.dismiss('vol')); }
        else { toast.loading('Atualizando...', { id: 'vol' }); atualizarVolume({ variables: { volumeId: initialData!.id, input, commentary } }).finally(() => toast.dismiss('vol')); }
    };
            ...
            <li className="card-std" style={{ width: '98%', margin: '10px 1%', padding: '1.5rem' }}>
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div><label className="block text-sm font-semibold">Status</label><select name="status" value={formData.status} onChange={handleFormChange} className="input-std mt-1">{Object.values(StatusVolume).map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                        <div><label className="block text-sm font-semibold">Mês</label><select name="m" value={formData.m} onChange={handleFormChange} className="input-std mt-1">{Object.values(MesVolume).map(m => <option key={m} value={m}>{m}</option>)}</select></div>
